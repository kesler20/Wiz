...
def update_plot(plot_data, columns, click_data, x_type, y_type, datatype):
...
def blank_layout():
    return go.Layout(
            autosize = True,
            xaxis = axis_layout_2D(['', ''], 'linear'),
            yaxis = axis_layout_2D(['Closing Price (USD)', ''], 'linear'),
            font =  dict(
...
def lines_type1_layout(df_clean, x_type, y_type, click_data):
    ...
    x_column =  ['', df_clean.dtypes[0].name]
    y_column = ['Closing Price (USD)', df_clean.dtypes[1].name]

...
@app.callback(
    [
        Output('example-1-x-select','options'),
        Output('example-1-y-select','options'),
        Output('example-1-z-select','options'),
        Output('example-1-c-select','options'),
        Output('example-1-s-select','options'),
        Output('example-1-x-select','value'),
        Output('example-1-y-select','value'),
        Output('example-1-z-select','value'),
        Output('example-1-c-select','value'),
        Output('example-1-s-select','value'),
        Output('example-1-main-graph','clickData'),
    ],
    [
        Input('example-1-slider','value'),
    ],
    [
        State('example-1-x-select','value'),
        State('example-1-y-select','value'),
        State('example-1-z-select','value'),
        State('example-1-c-select','value'),
        State('example-1-s-select','value')
    ]
)
def populate_dropdowns(sheet_num, x_value, y_value, z_value, c_value, s_value):
    # Get data
    data = glob_data.example_1
...
@app.callback(
    Output('example-1-z-select','disabled'),
    [
        Input('example-1-graph-type', 'value'),
    ]
)
...
@app.callback(
    [
        Output('example-1-main-graph','figure'),
        Output('example-1-main-graph','animate'),
        Output('example-1-main-graph','style'),
        Output('example-1-main-graph','className'),
        Output('example-1-display-size-scale','style'),
        Output('example-1-min-size','children'),
        Output('example-1-max-size','children'),
        Output('example-1-lastplot','children'),
        Output('example-1-interval','interval'),
    ],
    [
        Input('example-1-x-select','value'),
        Input('example-1-y-select','value'),
        Input('example-1-c-select','value'),
        Input('example-1-s-select','value'),
        Input('example-1-z-select','value'),
        Input('example-1-z-select','disabled'),
        Input('example-1-x-axis-type','value'),
        Input('example-1-y-axis-type','value'),
        Input('example-1-graph-type','value'),
        Input('example-1-main-graph','clickData'),
        Input('example-1-interval','n_intervals')
    ],
    [
        State('example-1-slider','value'),
        State('example-1-lastplot','children'),
        State('example-1-interval','interval'),
        State('example-1-session-id','children')
    ]
)
def update_graph(x_column, y_column, c_column, s_column, z_column, z_off,x_type, y_type, graph_type, click_data, intervals, sheet_num, last_plot, interval, session_id):
    # Get data
    data = glob_data.example_1
    ...
    if z_column is not None:
        x_range, y_range, z_range = get_ranges(data, x_column, y_column, z_column)
    else:
        x_range, y_range, z_range = get_ranges(data, x_column, y_column, x_column)
    ...
    n_total = len(list(sub_data.index)) - 1;
    multiplier = 150;
    if multiplier*intervals > len(sub_data.index):
        sub_data = sub_data.iloc[0:]
        interval = 10*60*60*1000
    else:
        sub_data = sub_data.iloc[0:multiplier*intervals]
    ...
    return out_figure, animate, out_style, className, display_style, min_size, max_size, new_plot, interval


# Update download link for plotted data
@app.callback(
    Output('example-1-download-link', 'href'),
    [
        Input('example-1-session-id','children')
    ],
)

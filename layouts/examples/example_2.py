...
from layouts.examples.example_2_layout import app_navbar, top_row, size_scale, graph, axis_select
...
x_range, y_range, z_range = get_ranges(all_data, 'linear','linear', columns[1], columns[2], columns[5])
...
slider = dbc.Col(
    [
        html.Div(
            ...
            id = 'example-2-slider-call',
            className = 'slider'
        )
    ],
    width = dict(size = 6, offset = 3),
    id = 'example-2-filter-slider'
)
...
out = [
    ...
    slider,
    data_table.get_table('example-2-data-table-plot'),
    html.Div([],id='example-2-lastplot', style=dict(display = 'none')),
    html.Div(genSession(),id='example-2-session-id', style=dict(display='none')),
]
